...
const QOS = () => {
    document.title = "QOS Policies | Minia";
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            setError("");
            const res = await getData(`/qosPolicies/meta`);
            setPolicies(res || []);
        } catch (err) {
    ...
    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="QOS Policies" breadcrumbItem="QOS Policies" />

                {/* Add Button */}
                <div className="mb-3">
                    <Button color="primary" onClick={() => navigate("/create-edit-qos")}>
                        New QOS Policy
                    </Button>
                </div>
                                    ...
                                    <tr key={policy._id}>
                                        ...
                                        <td>
                                            ...
                                            <Button
                                                ...
                                                id={`edit-${policy._id}`}
                                                onClick={() =>
                                                    navigate("/create-edit-qos", { state: { policy } })
                                                }
                                            >
                                                <FaEdit />
                                            </Button>
